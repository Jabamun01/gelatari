...
export const createIngredientHandler = async (req: Request, res: Response) => {
  try {
    ...
    const ingredientData = {
        name: name.trim(),
        // Default isAllergen to false if not provided or explicitly null/undefined
        isAllergen: typeof isAllergen === 'boolean' ? isAllergen : false,
    };

    // Check for existing ingredient with the same name (case-insensitive)
    const existingIngredient = await Ingredient.findOne({
      name: { $regex: `^${ingredientData.name}$`, $options: 'i' } // Case-insensitive regex match
    });

    if (existingIngredient) {
      return res.status(409).json({ message: `Ingredient with name "${ingredientData.name}" already exists.` });
    }

    // If no duplicate, proceed to create
    const newIngredient = await ingredientService.createIngredient(ingredientData.name, ingredientData.isAllergen);
    ...
    if (error && error.code === 11000) { // Add null check for error
        // This might still happen in rare race conditions if the index is case-sensitive
        return res.status(409).json({ message: `Ingredient with name "${req.body.name}" already exists (database constraint).` });
    }
...
export const getAllIngredientsHandler = async (req: Request, res: Response) => {
  try {
    ...
    const result = await ingredientService.getAllIngredients(name, page, limit);

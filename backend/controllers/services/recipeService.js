...
exports.getDependentParentRecipes = exports.finalizeRecipeProduction = exports.deleteRecipe = exports.updateRecipe = exports.getRecipeById = exports.getAllRecipes = exports.createRecipe = void 0;
...
const deleteRecipe = async (id) => {
    try {
        ...
        const dependentRecipes = await Recipe_1.default.find({
            'linkedRecipes.recipe': new mongoose_1.Types.ObjectId(id),
        })
        ...
        if (dependentRecipes.length > 0) {
            console.warn(`Recipe with ID ${id} is a dependency for other recipes:`, dependentRecipes.map(r => r.name));
            return {
                dependencies: dependentRecipes.map(r => ({
                    _id: new mongoose_1.Types.ObjectId(r._id.toString()), // Convert to mongoose.Types.ObjectId
                    name: r.name,
                })),
            };
        }
 ...
 */
const getDependentParentRecipes = async (recipeId) => {
    try {
        if (!mongoose_1.Types.ObjectId.isValid(recipeId)) {
        ...
        const parentRecipes = await Recipe_1.default.find({
            'linkedRecipes.recipe': new mongoose_1.Types.ObjectId(recipeId),
        })
            ...
            .exec();
        return parentRecipes; // Cast if lean() is used and IRecipe is expected
    }
    catch (error) {
...
exports.getDependentParentRecipes = getDependentParentRecipes;
